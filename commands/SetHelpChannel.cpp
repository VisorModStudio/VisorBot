#include "SetHelpChannel.h"
#include "../BotClient.h"
#include <sqlite3.h>
#include <string>
#include <dpp/dpp.h>

std::string SetHelpChannel::getName() const
{
    return "sethelpchannel";
}
...
void SetHelpChannel::execute(const dpp::slashcommand_t& event)
{
    ...
    std::string query = "INSERT INTO ServerConfig (GuildID, HelpChannelID) VALUES (?, ?) "
                     "ON CONFLICT(GuildID) DO UPDATE SET HelpChannelID = excluded.HelpChannelID;";
    sqlite3_stmt* stmt;
    ...
    if (sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        if (sqlite3_step(stmt) == SQLITE_DONE) {
            event.reply(dpp::message("HelpChannel got registered successfully.").set_flags(dpp::m_ephemeral));
        } else {
